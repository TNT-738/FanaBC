...
public class NewsLoader extends Fragment {
    ...
    public class SimpleRVAdapter extends RecyclerView.Adapter<SimpleViewHolder> {
        ...
        public void onBindViewHolder(@NotNull SimpleViewHolder holder, final int position) {
            ...
            if (pubDate != null) {
                holder.pubdate.setText(String.format("Published: %s", pubDate));
            } else {
                holder.pubdate.setText(String.format("Published: %s", "Unspecified"));
            }
            if (author != null) {
                holder.author.setText(String.format("Author: %s", author));
            } else {
                holder.author.setText(String.format("Author: %s", "Unspecified"));
            }
                ...
                public void onClick(View v) {
                    switch (dataSource.get(position).getArticleType()){
                        ...
                        case VIDEO:
                            ...
                            if(settingLanguage.equals(Lang_Type.LangCode.Amharic) ||
                                    settingLanguage.equals(Lang_Type.LangCode.Tigrigna)){
                             newVideoBrowser.putExtra("video_url", new Lang_Type(settingLanguage, getActivity()).getAllUrl().get(6).getUrl());
                            }
    ...
    private void setTag(Article.ArticleType articleType, SimpleViewHolder simpleViewHolder) {
        switch (articleType) {
            case VIDEO:
